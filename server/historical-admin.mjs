...
export function historicalAdminChange(previous,actor,input,now=new Date().toISOString()){
 ...
 if(input.previousStep!==Number(previous.journeyStep)||input.previousVersion!==Number(previous.nativeAgentArtifacts?.[input.document]?.version||0))throw fail(409,'과제가 변경되었습니다. 새로고침 후 다시 시도해 주세요.');
 ...
 if(input.action==='replace'){
  if(!['INT','FEA','ARD'].includes(input.document)||typeof input.name!=='string'||!input.name.toLowerCase().endsWith('.md')||typeof input.markdown!=='string'||!input.markdown.trim()||Buffer.byteLength(input.markdown)>1024*1024||input.markdown.includes('\u0000'))throw fail(400,'1MB 이하의 UTF-8 .md 문서를 첨부해 주세요.');
  event.document=input.document;event.previousVersion=input.previousVersion;
  if(input.document==='INT')state.intakeDraftCompleted=true;
  if(input.document==='FEA')state.feaCompleted=true;
...
export async function historicalAdminUpdate(identity,code,input,transact=withTransaction){
 return transact(async client=>{
  ...
  if(input.action==='replace'){
   ...
   event.version=doc.version_number;
  ...
  if(state.nativeAgentArtifacts?.ARD?.id&&!(input.action==='replace'&&input.document==='ARD'))await persistArdApprovalDocument(client,{...access.project,project_code:code},state,access.actor);
