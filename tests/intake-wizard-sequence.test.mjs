...
test("chat registration renders only the current INT section before advancing", () => {
  const page=readFileSync(new URL("../app/page.tsx",import.meta.url),"utf8");
  const editor=readFileSync(new URL("../app/intake-feasibility-v3.jsx",import.meta.url),"utf8");
  assert.match(page,/IntakeV3Fields sectionNumber=\{step\}/);
  assert.match(page,/stepRequired\.length > 0/);
