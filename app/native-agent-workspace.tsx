...
export default function NativeAgentWorkspace({projectNo,document,onCompleted,devRole}:{projectNo:string;document:'INT'|'FEA'|'ARD';onCompleted:(projectCode?:string)=>void;devRole?:string}){
 ...
 useEffect(()=>{function receive(event:MessageEvent){if(event.origin===window.location.origin&&event.source===frame.current?.contentWindow&&event.data?.type==='native-agent-completed'&&event.data.project===projectNo)onCompleted(event.data.projectCode||projectNo);}window.addEventListener('message',receive);return()=>window.removeEventListener('message',receive);},[projectNo,onCompleted]);
 ...
 if(access.mode==='final')return finalView||<p>작성 최종본을 불러오고 있습니다.</p>;
