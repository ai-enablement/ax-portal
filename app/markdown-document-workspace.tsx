...
function storedPhaseComplete(project:WorkspaceProject,phase:Phase){
  const code=phase==='design'?'DES':'EVD',record=project.markdownDocuments?.[code]?.phases?.[phase];
  return Number(record?.version)>0&&(record?.status===undefined||record.status==='complete');
}

export default function MarkdownDocumentWorkspace({project,phase,canEdit,devRole,onComplete}:{project:WorkspaceProject;phase:Phase;canEdit:boolean;devRole?:string;onComplete:(phase:Phase)=>Promise<boolean|void>|boolean|void}) {
  ...
  const requiredDocument=phase==='design'?'DES':'EVD';
  ...
  async function completePhase(){setBusy(true);setMessage('');try{const ok=await onComplete(phase);if(ok===false)throw new Error('단계 완료 조건을 확인해 주세요.');setPhaseComplete(true);setMessage(`${phaseLabels[phase]} 작성이 완료되어 다음 ${phase==='design'?'단계':'Gate'}로 이동했습니다.`);window.dispatchEvent(new Event('portal-agent-saved'));}catch(error){setMessage(error instanceof Error?error.message:'단계를 완료하지 못했습니다.');}finally{setBusy(false);}}
  return <section className="markdown-workspace">
    <header><div><small>{project.no} · {phaseLabels[phase]} 문서</small><h3>{phase==='design'?'설계 문서 작성':phase==='development_evaluation'?'개발·평가 문서 작성':'배포·확산 문서 작성'}</h3><p>.md 버전을 여러 번 첨부할 수 있습니다. 최종 버전을 올린 뒤 완료 버튼을 눌러야 다음 절차로 이동합니다.</p></div><div className="markdown-header-actions"><span className={phaseComplete?'complete':'pending'}>{phaseComplete?'작성 완료':finalVersion?'완료 확인 필요':'문서 필요'}</span><a href={`/api/markdown-documents/export?project=${encodeURIComponent(project.no)}&phase=${phase}`} download>앞 단계 누적 .md 다운로드</a></div></header>
    ...
    <footer><div><span>{active==='UG'?'UG는 선택 문서입니다.':'파일 첨부만으로 단계가 완료되지 않습니다.'}</span><p role="status">{message}</p></div>{canEdit&&active!=='UG'&&<button type="button" className="markdown-complete-button" disabled={busy||!finalVersion||phaseComplete} onClick={()=>void completePhase()}>{phaseComplete?'작성 완료':phase==='design'?`DES v${finalVersion?.version||'-'} 최종 확정 · 개발·평가로`:phase==='development_evaluation'?`EVD v${finalVersion?.version||'-'} 최종 확정 · G3 요청`:`EVD v${finalVersion?.version||'-'} 최종 확정 · G4 요청`}</button>}</footer>
  </section>;
}
