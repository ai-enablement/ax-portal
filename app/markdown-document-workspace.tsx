...
type Phase = "design" | "development_evaluation" | "deployment_rollout";
type HistoryItem = { id:string; documentType:"DES"|"EVD"|"UG"; phase:Phase; version:number; name:string; size:number; checksum:string; createdAt:string; authorName:string };
type OpenDocument = HistoryItem & { markdown:string };

const phaseLabels:Record<Phase,string> = { design:"설계", development_evaluation:"개발·평가", deployment_rollout:"배포·확산" };
const documentLabels = { DES:"에이전트 설계서[DES]", EVD:"개발·평가 문서[EVD]", UG:"사용자 가이드[UG]" } as const;
...
export default function MarkdownDocumentWorkspace({project,phase,canEdit}:{project:{no:string;name:string};phase:Phase;canEdit:boolean}) {
  const codes = phase==='design'?['DES'] as const:phase==='development_evaluation'?['EVD'] as const:['EVD','UG'] as const;
  ...
  const load=useCallback(async()=>{const response=await fetch(`/api/markdown-documents?project=${encodeURIComponent(project.no)}`,{cache:'no-store'});const payload=await response.json();if(!response.ok)throw new Error(payload.error||'문서 이력을 불러오지 못했습니다.');return payload.documents||[];},[project.no]);
  useEffect(()=>{let active=true;void load().then(documents=>{if(active)setHistory(documents);}).catch(error=>{if(active)setMessage(error.message);});return()=>{active=false;};},[load]);
  const visible=useMemo(()=>history.filter(item=>item.documentType===active),[history,active]);
  const phaseComplete=history.some(item=>item.documentType===active&&item.phase===phase);
  async function open(item:HistoryItem){setBusy(true);setMessage('');try{const response=await fetch(`/api/markdown-documents/${item.id}`,{cache:'no-store'});const payload=await response.json();if(!response.ok)throw new Error(payload.error||'문서를 열지 못했습니다.');setOpened(payload.document);}catch(error){setMessage(error instanceof Error?error.message:'문서를 열지 못했습니다.');}finally{setBusy(false);}}
  async function upload(file:File){setBusy(true);setMessage('');try{const form=new FormData();form.set('project',project.no);form.set('document',active);form.set('phase',phase);form.set('file',file);const response=await fetch('/api/markdown-documents',{method:'POST',body:form});const payload=await response.json();if(!response.ok)throw new Error(payload.error||'문서를 저장하지 못했습니다.');setHistory(await load());await open(payload.document);setMessage(`${documentLabels[active as keyof typeof documentLabels]} v${payload.document.version}을 저장했습니다. 게이트 승인은 별도로 진행해야 합니다.`);window.dispatchEvent(new Event('portal-agent-saved'));}catch(error){setMessage(error instanceof Error?error.message:'문서를 저장하지 못했습니다.');}finally{setBusy(false);}}
  return <section className="markdown-workspace">
    <header><div><small>{project.no} · {phaseLabels[phase]} 문서</small><h3>{phase==='design'?'설계 문서 작성':phase==='development_evaluation'?'개발·평가 문서 작성':'배포·확산 문서 작성'}</h3><p>.md 원본을 첨부하면 해당 문서가 작성 완료됩니다. 재첨부 시 이전 버전은 보존됩니다.</p></div><div className="markdown-header-actions"><span className={phaseComplete?'complete':active==='UG'?'complete':'pending'}>{phaseComplete?'작성 완료':active==='UG'?'선택 문서':'문서 필요'}</span><a href={`/api/markdown-documents/export?project=${encodeURIComponent(project.no)}&phase=${phase}`} download>앞 단계 누적 .md 다운로드</a></div></header>
