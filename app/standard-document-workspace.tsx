...
import { ARD_INHERITED_FIELD_KEYS, hydrateStandardDocuments, sectionHasContent, stageDocumentCodes, standardDocuments } from "../shared/standard-documents.mjs";
...
export default function StandardDocumentWorkspace({ project, stage, record, canEdit, onSave, onGallerySubmit, people = [], allowPartialSave = false, initialDocument }: {
  ...
  const isSectionComplete = (item: (typeof definition.sections)[number]) => document.completedSections.includes(item.id) || sectionHasContent(item, document.fields);
  const sectionHasSavedValue = (item: (typeof definition.sections)[number]) => item.fields.some((field) => {
    const value = document.fields[`${item.id}.${field.id}`];
    if (typeof value === "string") return Boolean(value.trim());
    if (Array.isArray(value)) return value.some((entry) => typeof entry === "boolean" ? entry : entry && (typeof entry !== "object" || Object.values(entry).some((cell) => String(cell || "").trim())));
    return Boolean(value && typeof value === "object" && contentText(value).trim());
  });
  const completed = definition.sections.filter(isSectionComplete).length;
  ...
  const save = async (complete: boolean) => {
    ...
    next.documents[code].completedSections = [...new Set([
      ...next.documents[code].completedSections,
      ...definition.sections.filter((item) => sectionHasContent(item, next.documents[code].fields)).map((item) => item.id),
    ])];
  ...
  return <div className="standard-document-shell">
    ...
    <div className="intake-result-layout draft standard-document-workspace">
      <section ref={panel} className="intake-document ard-document selectable-document" aria-label={`${definition.title} 표준 양식`}>
        ...
        {code === "ARD" && <section className="ard-inheritance-summary" aria-label="INT FEA 승계 정보"><div><small>FEA 트랙</small><b>{String(document.fields["autonomy.track"] || "미선택")}</b></div><div><small>Agent 유형</small><b>{String(document.fields["overview.agentType"] || "미선택")}</b></div><div><small>자율성 초안</small><b>{String(document.fields["autonomy.level"] || "미선택")}</b></div><p>INT/FEA 내용을 바탕으로 ARD 초안을 채웠습니다. 작성 담당자가 승계값과 세부 요구사항을 확인·수정한 뒤 문서 작성을 완료하면 G2 승인 요청으로 넘어갑니다.</p></section>}
        ...
        <nav className="ard-section-nav" aria-label={`${code} 문서 항목`}>{definition.sections.map((item, index) => { const done = isSectionComplete(item); const started = !done && sectionHasSavedValue(item); return <div className="ard-section-item" key={item.id}><button type="button" className={active === index ? "active" : ""} aria-expanded={active === index} onClick={() => selectSection(index)}><span className={`section-check ${done ? "complete" : started ? "started" : "pending"}`}>{done ? <Check size={14} weight="bold" /> : <FileText size={14} />}</span><div><small>{String(index + 1).padStart(2, "0")}</small><b>{item.title}</b><p>{item.description}</p></div><em>{done ? "작성 완료" : started ? "작성 중" : "입력 필요"}</em><ArrowRight size={14} /></button>{active === index && <section className="standard-active-section" aria-label={item.title}><div className="ard-section-head"><span>{String(index + 1).padStart(2, "0")}</span><div><b>{item.title}</b><small>{item.description}</small></div></div>{item.fields.map(field => renderField(field, item.id))}{canEdit && <button type="button" className="secondary" disabled={!sectionHasContent(item, document.fields) || saving} onClick={() => { setDraft((previous) => ({ ...previous, documents: { ...previous.documents, [code]: { ...previous.documents[code], completedSections: [...new Set([...previous.documents[code].completedSections, item.id])] } } })); setDirty(true); setActive(null); }}>이 항목 작성 완료</button>}</section>}</div>; })}</nav>
        <footer><span role="status">{feedback || (dirty ? "변경사항을 저장해 주세요." : "항목별 입력 내용과 작성 상태를 DB에 저장합니다.")}</span>{canEdit && <div><button type="button" className="secondary" disabled={saving} onClick={() => void save(false)}>{saving ? "저장 중…" : "임시 저장"}</button><button type="button" disabled={saving} onClick={() => void save(true)}>{allowPartialSave ? "이관 내용 저장" : code === "ARD" ? "ARD 확인 완료 · G2 요청" : "문서 작성 완료"} <ArrowRight size={14} /></button></div>}</footer>
      </section>
