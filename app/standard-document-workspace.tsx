...
export default function StandardDocumentWorkspace({ project, stage, record, canEdit, onSave }: {
  project: { no: string; name: string; intakeAnswers?: string[] };
  stage: number;
  record?: StandardStageRecord;
  canEdit: boolean;
  onSave: (record: StandardStageRecord) => Promise<boolean | void> | void;
}) {
  const [draft, setDraft] = useState(() => hydrateStandardDocuments(stage, record, project));
  ...
  return <div className="standard-document-shell">
    ...
    <div className="intake-result-layout draft standard-document-workspace">
      <section ref={panel} className="intake-document ard-document selectable-document" aria-label={`${definition.title} 표준 양식`}>
        ...
        {section && <section className="standard-active-section" aria-label={section.title}><div className="ard-section-head"><span>{String(active! + 1).padStart(2, "0")}</span><div><b>{section.title}</b><small>{section.description}</small></div></div>{section.fields.map(renderField)}{canEdit && <button type="button" className="secondary" disabled={!sectionHasContent(section, document.fields) || saving} onClick={() => { setDraft((previous) => ({ ...previous, documents: { ...previous.documents, [code]: { ...previous.documents[code], completedSections: [...new Set([...previous.documents[code].completedSections, section.id])] } } })); setDirty(true); setActive(null); }}>이 항목 작성 완료</button>}</section>}
