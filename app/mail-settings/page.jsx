...
export default function MailSettings(){
  ...
  return <main style={{maxWidth:760,margin:'48px auto',padding:24,color:'#17385e',background:'#fff',border:'1px solid #d8e2ee',borderRadius:12}}>
    ...
    {state.worker&&<section aria-label="자동 메일 점검 상태" style={{padding:16,border:'1px solid #d8e2ee',borderRadius:8,marginBottom:20}}>
      <h2 style={{fontSize:18}}>자동 메일 점검 상태</h2>
      <p>현재 서버: {{unknown:'아직 실행 기록 없음',running:'점검 중',success:'정상 완료',failed:'오류 발생',retrying:'재시도 대기',locked:'다른 서버에서 실행 중'}[state.worker.status]||state.worker.status}</p>
      <p>최근 실행: {time(state.worker.startedAt)}<br/>최근 정상 완료: {time(state.worker.lastSuccessAt)}<br/>다음 재시도: {time(state.worker.nextRetryAt)}</p>
      <p>DB 최근 담당자 점검: {time(state.worker.latest_actor_scan_at)}<br/>DB 가장 오래된 담당자 점검: {time(state.worker.oldest_actor_scan_at)}</p>
      {state.mode!=='off'&&state.worker.stale&&<p role="alert" style={{color:'#b42318'}}>90분 이상 DB 점검 기록이 없습니다. 서버 로그와 연결 상태를 확인해 주세요.</p>}
      {state.worker.failures?.length>0&&<ul>{state.worker.failures.map((failure,i)=><li key={i}>{stages[failure.stage]||failure.stage} · {failure.code}{failure.actorId?` · 담당 계정 ID ${failure.actorId}`:''}</li>)}</ul>}
      <p style={{fontSize:13}}>정상 점검은 1시간마다 실행합니다. 현재 서버 실행 기록은 재시작 시 초기화되며, DB 담당자 점검 시각은 전체 발송 성공을 의미하지 않습니다.</p>
      <button onClick={()=>refresh().catch(()=>setMessage('연결을 확인해 주세요.'))} style={{padding:'8px 14px',border:'1px solid #173d6b',borderRadius:6}}>상태 새로고침</button>
    </section>}
