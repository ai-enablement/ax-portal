...
export function WorkflowControls({project,identity,people,onSave}){
  ...
  return <section className="workflow-v31-panel"><h3>{step<=6?'설계·개발·평가 진행 및 배포 조건':'파일럿 결과 및 확산 조건'}</h3>{step===5&&<div className="workflow-v31-actions"><span>{project.deliveryPhase==='development'?'⑤ 개발·평가 진행 중':'④ 설계 진행 중 · DES .md 첨부 후 개발·평가 시작'}</span>{author&&project.deliveryPhase!=='development'&&<button type="button" disabled={busy||!designDocumentComplete(project)} onClick={()=>save({deliveryPhase:'development'})}>개발·평가 시작 →</button>}</div>}
    ...
    {author&&step===5&&project.deliveryPhase==='development'&&<button type="button" disabled={busy||!developmentEvdComplete(project)} onClick={()=>save({journeyStep:6})}>EVD 제출 → G3 승인 요청</button>}
    {author&&step===7&&<button type="button" disabled={busy||!releaseEvdComplete(project)||!project.gateChecks?.G4?.evidence} onClick={()=>save({journeyStep:8})}>배포·확산 EVD 제출 → G4 승인 요청</button>}
    <p role="status">{busy?'저장 중…':message}</p></section>;
}
