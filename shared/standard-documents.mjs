...
const narrative = (id, title, description) => s(id, title, description, [f("body", "작성 내용"), f("evidence", "근거 · 참조 문서"), f("reviewNote", "검토 의견")]);
export const standardDocuments = {
  ...
  DEP: { title: "배포 체크리스트", sections: [
    ...
    s("pilot", "파일럿 계획", "대상·기간·피드백·종료 기준", [f("pilotAudience", "파일럿 대상"), f("pilotPeriod", "파일럿 기간"), f("feedbackMethod", "피드백 수집 방법"), f("exitCriteria", "종료 기준"), f("rolloutPlan", "확산 계획")]),
    s("results", "파일럿 결과", "사용량·품질·사용자 의견", [f("pilotUsage", "사용 건수"), f("pilotUsageRate", "사용률"), f("pilotErrors", "오류·조치 내역"), f("pilotCriticalErrors", "치명 오류 건수"), f("pilotSatisfaction", "만족도"), f("pilotFeedback", "사용자 피드백"), f("pilotDecision", "결과 및 권고")]),
  ] },
  UG: { title: "사용자 가이드", sections: [
    s("overview", "Agent 소개", "이 Agent가 하는 일", [f("intro", "Agent 소개")]),
    s("scope", "하지 않는 일", "사용 범위 밖 요청", [f("outOfScope", "하지 않는 일")]),
    s("usage", "사용 방법", "진입·입력·결과 확인", [f("usageSteps", "사용 순서")]),
  ...
  OPS: { title: "운영 대장", sections: [
    s("owners", "운영 기본 정보", "유형·트랙·자율성·운영 책임", [f("type", "유형"), f("track", "트랙"), f("autonomy", "자율성 수준"), f("owner", "Project Owner"), f("operator", "개발/운영 담당자"), f("knowledgeOwner", "지식 담당자"), f("deployed", "배포일", "date"), f("status", "운영 상태", "select", ["운영", "일시중지", "폐기"])]),
...
export function sectionHasContent(section, fields) {
  return section.fields.every((field) => {
    ...
    if (field.kind.endsWith("-table")) {
      const columns = field.kind === "fr-table" ? ["ID", "기능", "입력 → 에이전트 행동 → 출력", "우선순위"] : ["실패 유형", "예시", "피해", "대응"];
